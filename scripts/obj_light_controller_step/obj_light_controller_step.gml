...
if (surface_exists(surf)) {
    if(is_on) {
        ...
        with (obj_ct_powerbeam)
            if(_power > 0) draw_circle(x + random_range(-1, 1), y + random_range(-1, 1), 4 + random_range(-1, 1), false);
            
        with (obj_p_ct_wired)
            if(_power > 0) draw_circle(x + random_range(-1, 1), y + random_range(-1, 1), 24 + random_range(-1, 1), false);
