...
var landing_item = inv_grid[# 0, selected_slot];
var pickup_item = inv_grid[# 0, pickup_slot];

if(pickup_slot != -1){
	if(mouse_check_button_pressed(mb_left)){
		if(landing_item == item.none){
		...
		} else if((landing_item == item.can and pickup_item == item.canOpener) or (landing_item == item.canOpener and pickup_item == item.can)){
			...
			inv_grid[# 0,pickup_slot] = landing_item;
			
			pickup_slot = -1;
		}
	}
}
else if(landing_item != item.none){
